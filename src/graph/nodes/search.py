...
def search_node(state: GraphState) -> GraphState:
    logger.info("Querying search backends for paper candidates")
    use_perplexity = bool(state.config.perplexity_api_key)
    perplexity = None
    if use_perplexity:
        logger.info("Using Perplexity as primary search backend")
    ...
    for claim in state.claims:
        ...
        for q in queries:
            if perplexity:
                try:
                    ...
                    logger.debug("Perplexity query '%s' returned %d results", q.query, len(results))
                except Exception as exc:
                    logger.warning("Perplexity search failed for query '%s': %s", q.query, exc)
            if state.config.semantic_scholar_api_key:
                try:
                    results = client.search_papers(q.query, state.config.top_k_per_query)
                    items.extend(results)
                    logger.debug("Semantic Scholar query '%s' returned %d results", q.query, len(results))
                except Exception as exc:
                    logger.warning("Semantic Scholar search failed for query '%s': %s", q.query, exc)
                    continue
        if state.seed_papers and state.config.enable_seed_expansion:
            logger.info("Expanding search using %d seed papers", len(state.seed_papers[:3]))
            for seed in state.seed_papers[:3]:
                try:
                    ...
                    logger.debug("Seed expansion for %s returned %d papers", seed.bibkey, len(related))
                except Exception as exc:
                    logger.warning("Seed expansion failed for %s: %s", seed.bibkey, exc)
        ...
        logger.info("Claim %s: collected %d unique candidates (after deduplication)", claim.cid, final_count)
